...
$('.container#pa').css('height','200%');
$('.container#pb').css('height','350%');
$('#holder,#puzzle-a,#puzzle-b').css('height','100%');
$('#textpocket').css('height','99%');
//$('#puzzle-b>svg').css('transform','scale(4)');
// disappear nojs version
$('.container#nojs').css('display','none').css('height',0);
...
function scrollControl() {
	...
	if (status>2.9*vh) {$('.instructions').css('opacity',1);}
	else {$('.instructions').css('opacity',0);}

	// thicken/thin lines in puzzle-b partway through zoomout/in
	if (status<-0.8*vh) {$('#pb path').css('stroke-width',1);}
	else{$('#pb path').css('stroke-width',0.5);}

	// Switch from a to b when the top of the text pocket hits the top of the viewport on its way in
	if (status <= 0) {$('#puzzle-a').css('opacity',0);$('#puzzle-b').css('opacity',1);} 
	// Switch from b to a when it gets below the top of the viewport when scrolling back up
	else {$('#puzzle-a').css('opacity',1);$('#puzzle-b').css('opacity',0);}
	...
	if (status <= bannerthresh) {
		$('.slider').addClass('disappear');$('.banner').removeClass('disappear');$('svg').addClass('shift');
	} 
	// Switch from banner to slider when top of text pocket gets below that threshold when scrolling back up
	else {
		$('.banner').addClass('disappear');$('.slider').removeClass('disappear');$('svg').removeClass('shift');
	}
}
